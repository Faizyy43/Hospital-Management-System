...
const readScopedHospitalData = (hospitalId, key) => {
  try {
    return JSON.parse(localStorage.getItem(`hospital_admin_${hospitalId}_${key}`) || "[]");
  } catch {
    return [];
  }
};

const writeScopedHospitalData = (hospitalId, key, value) => {
  localStorage.setItem(`hospital_admin_${hospitalId}_${key}`, JSON.stringify(value));
};
...
export default function BookingModal({ hospital, onClose }) {
  ...
  const handleBooking = async () => {
    ...
    try {
      const response = await api.post(`/hospitals/${hospital.id}/book`, {
        doctor,
        date,
        slot,
      });
      ...
      const scopedAppointments = readScopedHospitalData(hospital.id, "appointments");
      writeScopedHospitalData(hospital.id, "appointments", [...scopedAppointments, appointment]);

      const scopedPatients = readScopedHospitalData(hospital.id, "patients");
      const patientExists = scopedPatients.some(
        (patient) =>
      ...
      if (!patientExists) {
        writeScopedHospitalData(hospital.id, "patients", [
          ...scopedPatients,
          {
            id: Date.now(),
            name: patientName,
            email: patientEmail,
            age: storedUser.age || "N/A",
            disease: "Appointment Booking",
      ...
      localStorage.setItem("appointments", JSON.stringify([...patientAppointments, appointment]));

      toast.success("Appointment booked successfully!");
      onClose();
    } catch (bookingError) {
      toast.error(
        bookingError.response?.data?.message || "Unable to book appointment right now.",
      );
    }
  };
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              >
                ...
                {hospital.doctors.map((doctorName) => (
