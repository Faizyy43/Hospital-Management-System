...
import { getPublicHospitals } from "../services/hospitalDirectoryService";
...
const parseAddress = (address = "") => {
  ...
  const state = statePart.split("-")[0].trim() || "Unknown State";

  return { city, state };
};

const formatHospital = (hospital) => {
  const { city, state } = parseAddress(hospital.address);
  ...
  return {
    ...
    doctors,
    logo: "",
    opdCategories: Array.isArray(hospital.opdCategories)
      ? hospital.opdCategories
      : [],
  };
};
...
export default function Hospitals() {
  ...
  useEffect(() => {
    ...
    const loadHospitals = async () => {
      try {
        ...
        if (isMounted) {
          setHospitals(response.map(formatHospital));
        }
      } catch (loadError) {
        if (isMounted) {
          setError(
  ...
  }, [hospitals, search, stateFilter, cityFilter]);
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 max-w-2xl">
                Browse our curated directory of certified hospitals and
                specialty clinics. Save facilities or securely book outpatient
                visits directly.
              </p>
            </div>
          ...
          <AnimatePresence mode="popLayout">
            {loading ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="col-span-full py-20 text-center bg-white border border-slate-200 rounded-xl"
              >
                <h3 className="text-base font-bold text-slate-900">
                  Loading hospitals...
                </h3>
                <p className="text-sm text-slate-500 mt-1">
                  Please wait while we fetch approved facilities.
                </p>
              </motion.div>
            ) : filtered.length > 0 ? (
              filtered.map((h, index) => {
                const isFav = storedUser
              ...
              >
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Adjust your search or filter criteria to view more locations.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
